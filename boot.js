...
(() => {
  ...
  function showSkeleton(){
    const node=splash();
    if(node&&!node.querySelector('.jh-boot-skeleton')&&!node.querySelector('.jh-boot-failed'))node.innerHTML=skeleton;
  }
  ...
  function fail(message='Não foi possível iniciar o JuHelo.'){
    ...
    failed=true;
    stopWatchdog();
    node.innerHTML=`<div class="jh-boot-failed" role="alert">
  ...
  function startWatchdog(){
    timeoutId=setTimeout(()=>fail('A conexão ou a sessão demorou além do esperado.'),15000);
  ...
  function loadScript(src,timeout=7000){
    return new Promise((resolve,reject)=>{
      ...
      function finish(ok,err){
        ...
        if(!ok)s.remove();
        ok?resolve():reject(err||new Error(`Falha: ${src}`));
      }
      s.src=src;s.async=true;s.crossOrigin='anonymous';
      s.onload=()=>finish(true);
      s.onerror=()=>finish(false,new Error(`Falha: ${src}`));
  ...
  (async()=>{
    try{
      ...
      console.error('JuHelo boot',error);
