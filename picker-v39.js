/* JuHelo v39 — picker próprio sem abrir o select nativo. */
(() => {
  let overlay = null;
  let pendingValue = '';
  ...
  function selectFromTarget(target) {
    if (!(target instanceof Element) || target.closest('.jh-picker-overlay')) return null;
  ...
  function openPicker(select) {
    ...
    overlay.innerHTML = `
      <section class="jh-picker-sheet" role="dialog" aria-modal="true" aria-label="${esc(pickerTitle(select))}">
        <header class="jh-picker-head">
          <div>
            <strong>${esc(pickerTitle(select))}</strong>
        ...
        <div class="jh-picker-list" role="listbox">
          ${options.map(o => `
            <button type="button" class="jh-picker-item ${o.value === select.value ? 'is-selected' : ''}" data-value="${esc(o.value)}" role="option" aria-selected="${o.value === select.value ? 'true' : 'false'}">
  ...
  function intercept(event) {
    ...
    neutralize(select);
    if (event.type === 'pointerdown' || event.type === 'touchstart') openPicker(select);
  }

  // O select nativo nunca recebe o toque. O campo visual pai é quem abre nosso picker.
