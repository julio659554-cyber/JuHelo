...
function memberAvatar(member, index) {
  const current = member?.user_id === state.session?.user?.id;
  const name = member?.profile?.display_name || (current ? authName() : 'Pessoa');
  const avatarUrl = member?.profile?.avatar_url;
  const body = avatarUrl
    ? `<img src="${esc(avatarUrl)}" alt="${esc(name)}" loading="lazy">`
    : esc(initials(name));
  return `<span class="avatar couple-avatar couple-avatar-${index + 1}${current ? ' current' : ''}" title="${esc(name)}" aria-label="${esc(name)}">${body}</span>`;
}
function topbarAvatars() {
  const members = state.members?.length
    ? state.members.slice(0, 2)
    : [{ user_id: state.session?.user?.id, profile: { display_name: authName(), avatar_url: state.profile?.avatar_url || null } }];
  return `<span class="couple-avatar-stack" aria-label="${members.length > 1 ? 'Contas do casal conectadas' : 'Conta conectada'}">
    ${members.map((member, index) => memberAvatar(member, index)).join('')}
    ${members.length > 1 ? '<span class="couple-connected-dot" title="Casal conectado"></span>' : ''}
  </span>`;
}
...
function topbar() {
  return `<header class="topbar">
    ...
    <div class="user-chip">
      <span class="profile-name">${esc(authName())}</span>
      <div class="avatar single-avatar" aria-hidden="true">${esc(initials(authName()))}</div>
      ${topbarAvatars()}
    </div>
  </header>`;
}
...
function openBox(box = null) {
  modal(box ? 'Editar caixinha' : 'Nova caixinha',
    `<form id="f" class="form-grid">
      ...
      <div class="field"><label>Prazo (opcional)</label><input class="input" name="date" type="date" value="${esc(box?.target_date || '')}"><div class="row-sub">Data desejada para chegar no valor-alvo.</div></div>
...
async function loadAll() {
  const showLoading = !state.household;
  if (showLoading) {
    state.loading = true;
    render();
  }
...
window.juheloRefreshBoxes = async () => {
  await loadBoxes();
  await loadBoxMovements();
  render();
};
...
async function init() {
  ...
  supabase.auth.onAuthStateChange(async (event, nextSession) => {
    ...
    if (event === 'USER_UPDATED') {
      if (previousUserId === nextUserId && state.household) {
        await loadProfile();
        render();
      }
      return;
    ...
    if (event === 'SIGNED_IN') {
      if (previousUserId === nextUserId && state.household && !state.loading) return;
      await loadAll();
      return;
    }

    if (previousUserId !== nextUserId || !state.household) await loadAll();
  });
