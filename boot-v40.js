(() => {
  ...
  function syncSplash(){
    ...
    if(!splash.querySelector('.jh-boot-skeleton')&&!splash.querySelector('.jh-boot-failed')) splash.innerHTML=skeleton;
    if(!splashTimer&&!failed) splashTimer=setTimeout(()=>fail('A conexão ou a sessão demorou além do esperado.'),15000);
  }
  ...
  (async()=>{
    try{
      await loadSupabase();
      await import('./app-v31.js?v=40');
    }catch(err){
      console.error('JuHelo boot v40',err);
